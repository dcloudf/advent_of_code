...
fn main() {
    ...
    println!(
        "{}",
        parse_race_info(&file)
            .iter()
            .map(|info| info.number_ways_to_beat())
            .reduce(|x, y| x * y)
            .unwrap()
    );
    println!(
        "{}",
        parse_race_info_with_bad_kerning(&file).number_ways_to_beat()
    );
}
...
impl RaceInfo {
    fn number_ways_to_beat(&self) -> u64 {
        (0..self.time + 1)
            .filter(|delay| (self.time - delay) * (delay) > self.distance)
            .count() as u64
    }
}
...
mod tests {
    ...
    fn test_parse_race_info_with_bad_kerning() {
        assert_eq!(
            parse_race_info_with_bad_kerning("Time:      7  15   30\nDistance:  9  40  200"),
            RaceInfo {
                time: 71530,
                distance: 940200
            }
